...
const LoginPage = () => {
  ...
  const {isLoading, logIn, authMe} = useAuthStore();
  ...
  return (
    <div className="bg-gradient-to-br from-black to-gray-700 min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white/10 backdrop-blur-md rounded-xl md:p-8 p-4 shadow-2xl text-white">
        <h2 className="text-3xl font-bold text-center mb-6 flex gap-3 items-center justify-center">Welcome Back</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-white text-indigo-600 font-bold py-2 rounded-lg hover:bg-gray-100 cursor-pointer transition duration-300" >
             {isLoading ? "Login In..." : "Log In"}
          </button>
          <p className="text-center text-sm mt-4">
            Don't have an account? <Link to='/signup' className="underline text-white font-medium">Sign up</Link>
          </p>
        </form>
      </div>
    </div>
